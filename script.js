const palabras = ['ALURA', 'AHORCADO', 'ORACLE', 'BANANO', 'GATO', 'PERRO', 'LAPTOP'];

var palabra = palabras[generarNum()];
var intentos = 0;
var ingresadas = document.getElementById('ingresadas').getAttribute('placeholder');
var avance = '';
...
function nuevoJuego() {
    ...
    intentos = 0;
    document.getElementById('jugador').src = "img/intentos/intento" + intentos + ".png";
}
...
function agregarPalara() {
    ...
    if (nueva.length <= 8) {
        ...
        guardar();
        notificar("rgba(10, 56, 113, 0.404)", '<i class="fa-solid fa-check""></i> Palabra agregada con exito!', 2000);
        jugar();
    } else {
...
function avanzar() {
    ...
    if (avance == palabra) {
        //setTimeout('alert("Felicidades")', 100);
        notificar("rgba(10, 56, 113, 0.404)", '<i class="fa-solid fa-check""></i> Felicidades, Ganaste!', 2000);
    } else {
        avance = ''
    }
}
...
function agregarExtremidad() {
    ...
    if (intentos <= 10) {
        ...
        //alert('Perdiste');
        notificar("rgba(113, 10, 10, 0.404)", '<i class="fa-solid fa-xmark"></i> Lo siento, Perdiste', 2000);
    }
}

function registrarLetra(letra) {
    ingresadas = document.getElementById('ingresadas').getAttribute('placeholder') + " " + letra;
